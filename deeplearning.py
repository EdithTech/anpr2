...
def extract_text(image, bbox):
    ...
    text = pt.image_to_string(inverted_roi, lang='eng', config='--psm 6')
...
#     _, thresh_roi = cv2.threshold(cv2.cvtColor(roi, cv2.COLOR_RGB2GRAY), 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
#     text = pt.image_to_string(inverted_roi, lang='eng', config='--psm 6')
#     text = ''.join(filter(str.isalnum, text))
#     text = text.strip()
...
# def drawings(image, boxes_np, confidences_np, index, path):
#     text_list = []
#     for ind in index:
...
#             license_text = json.loads(response.text)
#             print(license_text)

#             text_width, text_height = cv2.getTextSize(str(license_text), cv2.FONT_HERSHEY_SIMPLEX, 0.7, 1)[0]
...
#             cv2.putText(image, str(license_text), (x, y+h+27), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
